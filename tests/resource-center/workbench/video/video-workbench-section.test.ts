...
assert.ok(sectionContent.includes('class="video-management__knowledge-point"'))
...
assert.ok(sectionContent.includes('class="video-management__feedback"'))
assert.equal(sectionContent.includes('已打开上传视频抽屉，可继续补齐资源文件与发布信息。'), false)
assert.equal(sectionContent.includes('管理教学视频资源'), false)
assert.ok(sectionContent.includes("import { createVideo, listVideos, updateVideo } from '@/api/video.ts'"))
