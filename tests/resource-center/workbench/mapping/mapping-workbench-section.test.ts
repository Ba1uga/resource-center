...
assert.ok(sectionSource.includes('() => ({ ...filters.value })'))
assert.ok(sectionSource.includes('const sessionStore = useMappingWorkbenchSessionStore()'))
assert.ok(sectionSource.includes('const filters = computed({'))
assert.ok(sectionSource.includes('get: () => sessionStore.filters'))
assert.ok(sectionSource.includes('set: (value) => sessionStore.patchFilters(value)'))
assert.ok(sectionSource.includes('const page = computed({'))
assert.ok(sectionSource.includes('get: () => sessionStore.page'))
assert.ok(sectionSource.includes('set: (value) => sessionStore.setPage(value)'))
assert.ok(sectionSource.includes('const records = ref<MappingRecord[]>(createLocalRecords())'))
...
assert.ok(sectionSource.includes('const feedback = ref<{'))
...
assert.ok(sectionSource.includes("confidenceLevel: filters.value.confidenceLevel === 'low' ? 'all' : 'low'"))
assert.ok(sectionSource.includes("overviewStatus: filters.value.overviewStatus === status ? 'all' : status"))
assert.ok(sectionSource.includes("sessionStore.patchFilters({"))
assert.ok(sectionSource.includes('sessionStore.reset()'))
assert.ok(sectionSource.includes('sessionStore.setPage(nextPage)'))
assert.ok(sectionSource.includes('sessionStore.setPage(resolveMappingPageAfterMutation({'))
...
assert.equal(sectionSource.includes('Legacy anchors'), false)
...
assert.ok(sectionSource.includes('@update-keyword="sessionStore.patchFilters({ keyword: $event })"'))
assert.ok(sectionSource.includes('@update-resource-type="sessionStore.patchFilters({ resourceType: $event })"'))
assert.ok(sectionSource.includes('@update-chapter="sessionStore.patchFilters({ chapter: $event })"'))
assert.ok(sectionSource.includes('@update-batch="sessionStore.patchFilters({ batchId: $event })"'))
assert.ok(sectionSource.includes('@update-review-status="sessionStore.patchFilters({ reviewStatus: $event })"'))
assert.ok(sectionSource.includes('@update-confidence-level="sessionStore.patchFilters({ confidenceLevel: $event })"'))
