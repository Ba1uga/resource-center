...
assert.equal(defaultViewModel.summaryCards[0]?.label, '总课件数')
assert.equal(defaultViewModel.summaryCards[0]?.value, String(coursewareRecords.length))
assert.equal(defaultViewModel.summaryCards[0]?.kind, 'filter')
assert.equal(defaultViewModel.summaryCards[0]?.interactive, true)
assert.equal(defaultViewModel.summaryCards[0]?.active, true)
assert.equal(defaultViewModel.summaryCards[1]?.label, '当前结果数')
assert.equal(defaultViewModel.summaryCards[1]?.value, String(coursewareRecords.length))
assert.equal(defaultViewModel.summaryCards[1]?.kind, 'info')
assert.equal(defaultViewModel.summaryCards[1]?.interactive, false)
assert.equal(defaultViewModel.summaryCards[1]?.active, false)
assert.equal(defaultViewModel.summaryCards.length, 2)
...
assert.equal(multiFilterViewModel.summaryCards[0]?.active, false)
assert.equal(multiFilterViewModel.summaryCards[1]?.value, '1')
