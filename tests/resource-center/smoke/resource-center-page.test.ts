...
async function main() {
  ...
  if (
    !/\.dashboard-main\s*\{[\s\S]*?padding-right:\s*0;[\s\S]*?overflow:\s*hidden;[\s\S]*?scrollbar-gutter:\s*auto;/i.test(
      pageStyles,
    )
  ) {
    throw new Error(
      'resource-center-page.css must keep dashboard-main as a fixed-height container and leave scrolling to inner workbench regions',
    )
  }
