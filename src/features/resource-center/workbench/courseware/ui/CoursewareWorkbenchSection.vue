...
import { coursewareRecords, currentCoursewareUploader } from '@/features/resource-center/workbench/courseware/model/courseware-workbench.fixtures.ts'
import {
...
const pageSize = 8
const records = ref<CoursewareRecord[]>([])
...
async function loadCoursewares() {
  ...
  try {
    const pageData = await listCoursewares({
      page: 1,
      pageSize: 200,
    })
    ...
    console.error(error)
    records.value = [...coursewareRecords]
...
async function saveDrawer() {
  ...
  try {
    if (drawerMode.value === 'edit' && drawerTargetId.value) {
      const updated = await updateCourseware(Number(drawerTargetId.value), {
      ...
      records.value = records.value.map((record) =>
        record.id === drawerTargetId.value ? updated : record,
      )
    } else {
      const created = await createCourseware({
        title: drawerDraft.title.trim(),
        course: drawerDraft.course.trim(),
        chapter: drawerDraft.chapter.trim(),
        type: drawerDraft.type,
    ...
    feedback.value = {
      tone: 'success',
      text: drawerMode.value === 'edit' ? '课件已更新。' : '课件已上传。',
    }
  ...
  <section class="courseware-management workbench-surface" :data-section="props.section.key">
    <div class="courseware-management__controls">
      ...
      <div
        ...
        class="courseware-management__feedback is-info"
        role="status"
      >
        后端连接异常，当前显示本地课件数据。
      </div>
    ...
    <div class="courseware-management__table-shell">
      <div v-if="isLoading && records.length === 0" class="courseware-management__empty-state">
        <strong>正在加载课件数据...</strong>
        <p>正在尝试连接后端服务，请稍候。</p>
      </div>
    ...
    <div v-if="drawerOpen" class="courseware-management__drawer-shell" @click.self="closeDrawer">
      <aside class="courseware-management__drawer-panel" role="dialog" aria-modal="true">
        ...
        <footer class="courseware-management__drawer-actions">
          <button type="button" class="courseware-management__drawer-action" @click="closeDrawer" :disabled="isSaving">取消</button>
          <button type="button" class="courseware-management__drawer-action primary" @click="saveDrawer" :disabled="isSaving">
            {{ isSaving ? '保存中...' : '保存课件' }}
          </button>
        </footer>
      </aside>
